...
def test_BGP_GR_TC_9_p1(request):
    ...
    logger.info("[Phase 1] : Test Setup"
                " [Restart Mode]R1-----R2[Helper Mode] Initiliazed  ")
    ...
    for addr_type in ADDR_TYPES:
        ...
        protocol = 'bgp'
        result = verify_rib(tgen, addr_type, dut, input_topo, \
                            next_hop, protocol, expected=False)
        assert result is not True, \
            "Testcase {} :Failed \n Error {}". \
                format(tc_name, result)
        logger.info(" Expected behavior: {}".\
    ...
    for addr_type in ADDR_TYPES:
        ...
        assert result is True, ("BGP Convergence after BGPd restart"
                                " :Failed \n Error:{}".format(result))
        ...
        result = verify_r_bit(tgen, topo, addr_type, input_dict,
                              dut="r1", peer="r2")
        assert result is True, \
            "Testcase {} :Failed \n Error {}". \
                format(tc_name, result)

        result = verify_f_bit(tgen, topo, addr_type, input_dict,
                              dut="r1", peer="r2", expected=False)
        assert result is not True, \
            "Testcase {} :Failed \n Error {}". \
                format(tc_name, result)
...
def test_BGP_GR_TC_17_p1(request):
    ...
    logger.info("[Phase 1] : Test Setup [Disable]R1-----R2[Restart] "
        "Initiliazed  ")
...
def test_BGP_GR_TC_43_p1(request):
    ...
    step("Verify that R1 keeps BGP routes in zebra and R2 retains"
    ...
    step("Verify that R1 flush all BGP routes from RIB & FIB and FIB and R2"
    ...
    step("Bring up BGPd on R1 and configure it as GR"
        " restarting node in global level")
    ...
    step("Verify that R1 keeps BGP routes in zebra and R2"
...
def test_BGP_GR_TC_44_p1(request):
    ...
    step("Configure R2 as GR restating node in global level and"
...
def test_BGP_GR_TC_46_p1(request):
    ...
    step("Configure R1 and R2 as GR restarting node in global"
        " and helper in per-Peer-level")
    ...
    step("Verify that R1 keeps the stale entries in RIB & FIB and R2 keeps stale entries in FIB using")
    ...
    step("Verify that R1 keeps the stale entries in FIB command and R2 keeps stale entries in RIB & FIB")
...
def test_BGP_GR_TC_47_p1(request):
    ...
    step("Verify that R1 keeps the stale entries in FIB and R2 keeps stale entries in RIB & FIB")
    ...
    step("Bring up BGP on R1 and remove Peer-level GR"
...
def test_BGP_GR_TC_48_p1(request):
    ...
    step("Configure R1 as GR restarting node in global level and"
...
def test_BGP_GR_TC_49_p1(request):
    ...
    step("Verify that R2 receives GR restarting capabilities"
...
def test_BGP_GR_TC_52_p1(request):
    ...
    step("Configure R1 as GR disabled node at per Peer-level for R2"
    ...
    step("Verify that R2 keeps the stale entries in FIB & R1 doesn't keep RIB & FIB entries.")
    ...
    step("Verify on R2 that R1 advertises GR capabilities as a helper node from global inherit")
    ...
    step("Verify that R2 keeps the stale entries in FIB & R1 keeps stale entries in RIB & FIB")
