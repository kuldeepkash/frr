...
def test_bgp_timers_functionality():
    ...
    # Api call to clear bgp, so timer modification would take plac
    clear_bgp_and_verify(tgen, topo, 'ipv4', 'r1')
...
def test_static_routes():
    ...
    input_dict = {
        "r1": {
            "static_routes": [
                {"network": "10.0.20.1/32", "no_of_ip": 9, \
                 "admin_distance": 100, "next_hop": "10.0.0.2",\
                 "tag": 4001}]
        }}
...
def test_admin_distance_for_existing_static_routes():
    ...
    result = modify_admin_distance_for_static_routes(tgen, topo,
                                                     input_dict)
    if result != True:
        assert False, "Testcase " + tc_name + " :Failed \n Error: {}".\
        format(result)
    ...
    result = verify_admin_distance_for_static_routes(tgen, 'ipv4',
                                                     input_dict)
    if result != True:
        assert False, "Testcase " + tc_name + " :Failed \n Error: {}".\
        format(result)
...
def test_advertise_network_using_network_command():
    ...
    result = advertise_networks_using_network_command(tgen, topo,
                                                      'ipv4', input_dict)
    if result != True:
        assert False, "Testcase " + tc_name + " :Failed \n Error: {}".\
        format(result)
...
def test_bgp_with_loopback_interface():
    ...
    for routerN in sorted(topo['routers'].keys()):
        for bgp_neighbor in topo['routers'][routerN]['bgp']\
            ['bgp_neighbors'].keys():
            # Adding ['dest_link'] = 'lo' key:value pair
            topo['routers'][routerN]['bgp']['bgp_neighbors']\
            [bgp_neighbor]['peer']['dest_link'] = 'lo'
            # Adding ['source_link'] = 'lo' key:value pair
            topo['routers'][routerN]['bgp']['bgp_neighbors']\
            [bgp_neighbor]['peer']['source_link'] = 'lo'
