...
1. Verify mroutes updated with correct OIL and IIF on DUT
2. Verify mroutes OIL and IIF updated correctly when receivers present inside
    and outside of DUT
3. Verify mroutes OIL and IIF updated correctly when source present inside
    and outside of DUT
4. Verify (S,G) getting timeout after stopping the traffic
5. Verify mroutes updated correctly after source interface shut/no shut
6. Verify mroutes updated correctly after receiver interface shut/no shut
7. Verify mroute updated correctly after sending IGMP prune and join
8. Verify mroute updated correctly after clear mroute
9. Verify (*,G) mroute entries after changing the RP configuration
10. Verify (*,G) and (S,G) after adding removing the PIM and IGMP config
    from DUT interfaces
11. Verify mroute entries after PIMD service stop and start
12. Verify mroute entries after FRR service stop and start
13. Verify mroute when DUT replicating towards both the uplinks
14. Verify changing RP address on DUT from Static to BSR , IIF and OIF
    updated correctly
15. Verify when mroute RPT and SPT path is difference
"""
...
jsonFile = "{}/multicast_pim_uplink_topo1.json".format(CWD)
try:
...
def test_mroutes_updated_with_correct_oil_iif_when_receiver_is_in_and_outside_DUT_p0(
    ...
    input_join = {"i7": topo["routers"]["i7"]["links"]["r4"]["interface"]}
    ...
    for src, src_intf in input_src.items():
        result = config_to_send_igmp_join_and_traffic(
            ...
            src,
            src_intf,
            GROUP_RANGE_1 + GROUP_RANGE_2,
            traffic=True,
        )
        assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)

        result = iperfSendTraffic(
            tgen, src, IGMP_JOIN_RANGE_1 + IGMP_JOIN_RANGE_2, 32, 2500
        )
        assert result is True, "Testcase {} : Failed Error: {}".format(tc_name, result)
...
def test_verify_mroutes_forwarding_p0(request):
    ...
    for recvr, recvr_intf in input_join.items():
        result = config_to_send_igmp_join_and_traffic(
            tgen, topo, tc_name, recvr, recvr_intf, GROUP_RANGE_3, join=True
        )
        assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)

        result = iperfSendIGMPJoin(tgen, recvr, IGMP_JOIN_RANGE_3, join_interval=1)
        assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)
    ...
    for src, src_intf in input_src.items():
        result = config_to_send_igmp_join_and_traffic(
            tgen, topo, tc_name, src, src_intf, GROUP_RANGE_3, traffic=True
        )
        assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)

        result = iperfSendTraffic(tgen, src, IGMP_JOIN_RANGE_3, 32, 2500)
        assert result is True, "Testcase {} : Failed Error: {}".format(tc_name, result)
...
def test_mroutes_updated_correctly_after_source_interface_shut_noshut_p1(request):
    ...
    step(
        "After shut verify upstream got expired using 'show ip pim "
    ...
    for retry in range(1, 11):
        result = verify_upstream_iif(
            tgen, "r1", "Unknown", source_i6, IGMP_JOIN_RANGE_1, expected=False
        )
        if result is not True:
            done_flag = True
        else:
    ...
    for retry in range(1, 11):
        result = verify_upstream_iif(
            tgen, "r4", "Unknown", source_i2, IGMP_JOIN_RANGE_1, expected=False
        )
        if result is not True:
            done_flag = True
        else:
...
def test_mroutes_updated_correctly_after_receiver_interface_shut_noshut_p1(request):
    ...
    input_src = {"i2": topo["routers"]["i2"]["links"]["r1"]["interface"]}
...
def test_mroutes_updated_after_sending_IGMP_prune_and_join_p1(request):
    ...
    kill_iperf(tgen, "i1", "remove_join")
...
def test_mroutes_updated_after_after_clear_mroute_p1(request):
    ...
    clear_ip_mroute(tgen, "r1")

    step("Clear ip mroute from r4")
    clear_ip_mroute(tgen, "r4")
...
def test_mroutes_updated_after_adding_removing_pim_igmp_config_p1(request):
    """
    TC_15 CHAOS-6:
    Verify (*,G) and (S,G) after adding removing the PIM and IGMP config
    from DUT interfaces
    """
    ...
    step("Remove ip igmp from receiver interface of DUT")
    ...
    step(
        "After removing igmp from CLI verify IGMP groups are removed "
        "using 'show ip igmp groups json'"
    )
    ...
    step(
        "(*,G) and (S,G) got removed from DUT using 'show ip mroute json'"
        "'show ip pim upstream' and traffic stopped 'show ip multicast json'"
    )
    ...
    step("Config ip igmp on receiver interface of DUT")
    ...
    step(
        "After config of igmp CLI all the groups are received verify using"
        " 'show ip igmp json'"
    )
    ...
    step(
        "(*,G) and (S,G) got re-learn from DUT using 'show ip mroute json'"
        " 'show ip pim upstream' and traffic resume 'show ip multicast json'"
    )
    ...
    step(
        "Remove PIM 'no ip pim sm' from all the DUT interface one by one"
        " ( DUT-R4, DUT-R2, DUT-R3)"
    )

    input_dict_1 = {"r1": {"pim": {"disable": r1_r2_links + r1_r3_links}}}
    ...
    result = verify_pim_interface(tgen, topo, "r1", expected=False)
    assert result is not True, (
        "Testcase {}: Failed "
        "PIM interfaces still present \n Error: {}".format(tc_name, result)
    )

    step("Corresponding nbr is down 'show ip pim nbrs json'")

    result = verify_pim_neighbors(tgen, topo, "r1", expected=False)
    assert (
        result is not True
    ...
    step("Enable PIM for all interfaces in DUT")

    input_dict_1 = {"r1": {"pim": {"enable": r1_r2_links + r1_r3_links}}}
    result = create_pim_config(tgen, topo, input_dict_1)
    assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)

    step("Nbr has come up after adding the PIM config 'show ip pim nbrs json'")

    result = verify_pim_neighbors(tgen, topo, "r2")
    assert result is True, "Testcase {}: Failed Error: {}".format(tc_name, result)

    step(
        "Adding PIM config for all the link (S,G) taking shortest path "
        "(DUT-R4) verify using 'show ip mroute json' 'show ip multicast json'"
    )
    ...
    step("Remove PIM config from all the uplinks")
...
def test_mroutes_when_dut_replicating_towards_uplinks_p1(request):
    """
    TC_18 CHAOS-9:
    Verify mroute when DUT replicating towards both the uplinks
    """
    ...
    step("Enable IGMP on DUT and R2, R3 interface")
    intf_r1_i1 = topo["routers"]["r1"]["links"]["i1"]["interface"]
    intf_r2_i3 = topo["routers"]["r2"]["links"]["i3"]["interface"]
    intf_r3_i5 = topo["routers"]["r3"]["links"]["i5"]["interface"]
    for dut, intf in zip(["r1", "r2", "r3"], [intf_r1_i1, intf_r2_i3, intf_r3_i5]):
        input_dict = {dut: {"igmp": {"interfaces": {intf: {"igmp": {"version": "2"}}}}}}
    ...
    step("Configure RP as R4 for group range 225.1.1.1-10")
    ...
    input_dict_star_sg = [
        ...
        },
        {
            ...
            "src_address": "*",
            "iif": r2_r4_links,
            "oil": r2_r1_links + [topo["routers"]["r2"]["links"]["i3"]["interface"]],
        },
        {
            "dut": "r4",
            "src_address": source_i6,
        ...
        },
        {
            ...
            "oil": r2_r1_links
            + r2_r4_links
            + [topo["routers"]["r2"]["links"]["i3"]["interface"]],
        },
    ]
    ...
    step("Send prune and join for received connected to R2")
    kill_iperf(tgen, "i3", "remove_join")
    step("Send prune and join for received connected to R3")
    kill_iperf(tgen, "i5", "remove_join")
    clear_ip_mroute(tgen)

    step(
        "Sending Prune from R2, verify R2 receivers not receiving the "
        "traffic , DUT has removed mroute OIF towards R2 , no impact seen "
        "on R3 receivers Verify using 'show ip mroute' 'show ip multicast' "
        "on DUT and R2"
    )

    step(
        "Sending Prune from R3 , verify R3 receivers not receiving the traffic"
        ", DUT has removed mroute OIF towards R3, no impact seen on R2 receivers"
    )
    ...
    step(
        "Mroute entry updated correctly and traffic started flowing after "
        "receiving the joins for R2 receivers show ip mroute json'"
    )
    ...
    step("Shut of R2 receiver connected port")
    intf_r2_i3 = topo["routers"]["r2"]["links"]["i3"]["interface"]
    shutdown_bringup_interface(tgen, "r2", intf_r2_i3, False)

    step("Shut of R3 receiver connected port")
    intf_r3_i5 = topo["routers"]["r3"]["links"]["i5"]["interface"]
    shutdown_bringup_interface(tgen, "r3", intf_r3_i5, False)

    step(
        "Shut receiver interface from R2 , verify R2 receivers not receiving "
        "the traffic , DUT has removed mroute OIF towards R2 , no impact seen "
        "on R3 receivers Verify using 'show ip mroute' show ip multicast' on "
        "DUT and R2"
    )

    step(
        "Shut receiver interface from R3 , verify R3 receivers not receiving the "
        "traffic , DUT has removed mroute OIF towards R3 , no impact seen on R2 "
        "receivers"
    )

    for data in input_dict_star_sg:
        if data["src_address"] == source_i4:
            result = verify_ip_mroutes(
                tgen,
                data["dut"],
                data["src_address"],
    ...
    step("No shut of R2 receiver connected port")
    intf_r2_i3 = topo["routers"]["r2"]["links"]["i3"]["interface"]
    shutdown_bringup_interface(tgen, "r2", intf_r2_i3, True)

    step("No shut of R3 receiver connected port")
    intf_r3_i5 = topo["routers"]["r3"]["links"]["i5"]["interface"]
    shutdown_bringup_interface(tgen, "r3", intf_r3_i5, True)

    step(
        "Mroute entry updated correctly and traffic started flowing after "
        "no shut of R2 receivers 'show ip mroute json'"
    )

    step(
        "Mroute entry updated correctly and traffic started flowing after "
        "no shut of R3 receivers 'show ip mroute json'"
    )
...
def test_mroutes_after_pimd_restart_p2(request):
    """
    TC_16 CHAOS-7:k
    Verify mroute entries after PIMD service stop and start
    """
    ...
    step(
        "Configure EBGP loopback session between all the nodes to "
        "make RP and source reachable"
    )

    for routerN in sorted(topo["routers"].keys()):
        if "bgp" in topo["routers"][routerN]:
    ...
    for i in range(1, 5):
        ipv4_list = [
            ...
            (
                "r1",
                topo["routers"]["r1"]["links"]["r3-link{}".format(i)]["interface"],
                [topo["routers"]["r3"]["links"]["lo"]["ipv4"]],
            ),
            (
                "r2",
                topo["routers"]["r2"]["links"]["r1-link{}".format(i)]["interface"],
                [topo["routers"]["r1"]["links"]["lo"]["ipv4"]],
            ),
            (
                "r2",
                topo["routers"]["r2"]["links"]["r4-link{}".format(i)]["interface"],
                [topo["routers"]["r4"]["links"]["lo"]["ipv4"]],
            ),
            (
                "r3",
                topo["routers"]["r3"]["links"]["r1-link{}".format(i)]["interface"],
                [topo["routers"]["r1"]["links"]["lo"]["ipv4"]],
            ),
            (
                "r3",
                topo["routers"]["r3"]["links"]["r4-link{}".format(i)]["interface"],
                [topo["routers"]["r4"]["links"]["lo"]["ipv4"]],
            ),
            (
                "r4",
            ...
            (
                "r4",
                topo["routers"]["r4"]["links"]["r3-link{}".format(i)]["interface"],
                [topo["routers"]["r3"]["links"]["lo"]["ipv4"]],
            ),
        ]
    ...
    step("Shut and no shut the receiver interface from DUT")
    for i in range(1, 5):
        intf = topo["routers"]["r1"]["links"]["r2-link{}".format(i)]["interface"]
        shutdown_bringup_interface(tgen, "r1", intf, False)
    ...
    step(
        "After shut of upstream interface from DUT verify mroutes has moved "
        "to another interface (R3) and updated with correct OIL/IIF using"
        " 'show ip mroute json'"
    )
    ...
    step("No shut of upstream interface from DUT side")
    for i in range(1, 5):
        intf = topo["routers"]["r1"]["links"]["r2-link{}".format(i)]["interface"]
        shutdown_bringup_interface(tgen, "r1", intf, True)
    ...
    step("PIM got restarted with new process id using 'pgrep pimd'")
    ...
    step("Stop the traffic and kill the PIM process")
    kill_iperf(tgen)
    ...
    step(
        "PIM process restarted and (S,G) mroute not present in R4 and DUT "
